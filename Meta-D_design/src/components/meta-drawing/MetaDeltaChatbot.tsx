    ...
    const initialMessage: ChatMessage = {
      ...
      actions: selectedPersona ? 
        ...
        [
          ...
          { id: 'action-3', label: 'P&ID 심볼 검증하기', type: 'navigate' }
        ]
    };
      ...
      if (selectedPersona.name === 'engineering_ai') {
        if (input.includes('heat exchanger') || input.includes('열교환기')) {
          return {
            id: `bot-${Date.now()}`,
            type: 'bot',
            content: `${personaPrefix}P&ID 도면을 분석했습니다. HE-101, HE-102, HE-103 총 3개의 열교환기가 확인되며, 설계 표준 TEMA에 따른 검증이 필요합니다. 각 열교환기의 열전달 계수와 압손을 ��인해드릴까요?`,
            timestamp: new Date(),
            actions: [
